...
class RCF(nn.Module):
    def __init__(self, cfg, writer):
        ...
        if self.cfg.MODEL.change_conv5_dsn5:
            ...
            # if use bilinear-weight to upsample, need to change the size of kernel

        # { add lstm module
        if self.cfg.MODEL.LSTM or self.cfg.MODEL.LSTM_bu:
            # lstm cells
            if self.cfg.MODEL.LSTM_version == 'v2':
                self.lstmcell_1 = ConvLSTMCell_v2(input_channels=21, hidden_channels=12, prediction_channels=1,
                                                  kernel_size=3)
                self.lstmcell_2 = ConvLSTMCell_v2(input_channels=21, hidden_channels=12, prediction_channels=1,
                                                  kernel_size=3)
                self.lstmcell_3 = ConvLSTMCell_v2(input_channels=21, hidden_channels=12, prediction_channels=1,
                                                  kernel_size=3)
                self.lstmcell_4 = ConvLSTMCell_v2(input_channels=21, hidden_channels=12, prediction_channels=1,
                                                  kernel_size=3)
                self.lstmcell_5 = ConvLSTMCell_v2(input_channels=21, hidden_channels=12, prediction_channels=1,
                                                  kernel_size=3)
            else:
    ...
    def forward(self, x):
        ...
        if self.cfg.MODEL.LSTM:
            ...
            if self.cfg.MODEL.upsample_layer == 'deconv':
                ...
                if self.cfg.MODEL.interpolate_mode == 'nearest':
                    dsn2_up = F.interpolate(dsn2, size=(2 * (h2 + 1), 2 * (w2 + 1)),
                                            mode=self.cfg.MODEL.interpolate_mode)
                elif self.cfg.MODEL.interpolate_mode == 'bilinear':
                    dsn2_up = F.interpolate(dsn2, size=(2 * (h2 + 1), 2 * (w2 + 1)),
                                            mode=self.cfg.MODEL.interpolate_mode, align_corners=True)
        ...
        if self.cfg.MODEL.LSTM:
            ...
            if self.cfg.MODEL.upsample_layer == 'deconv':
                ...
                h3, w3 = dsn3.shape[2:]
                if self.cfg.MODEL.interpolate_mode == 'nearest':
                    dsn3_up = F.interpolate(dsn3, size=(4 * (h3 + 1), 4 * (w3 + 1)),
                                            mode=self.cfg.MODEL.interpolate_mode)
                elif self.cfg.MODEL.interpolate_mode == 'bilinear':
                    dsn3_up = F.interpolate(dsn3, size=(4 * (h3 + 1), 4 * (w3 + 1)),
                                            mode=self.cfg.MODEL.interpolate_mode, align_corners=True)
        ...
        if self.cfg.MODEL.LSTM:
            ...
            if self.cfg.MODEL.upsample_layer == 'deconv':
                ...
                h4, w4 = dsn4.shape[2:]
                if self.cfg.MODEL.interpolate_mode == 'nearest':
                    dsn4_up = F.interpolate(dsn4, size=(8 * (h4 + 1), 8 * (w4 + 1)),
                                            mode=self.cfg.MODEL.interpolate_mode)
                elif self.cfg.MODEL.interpolate_mode == 'bilinear':
                    dsn4_up = F.interpolate(dsn4, size=(8 * (h4 + 1), 8 * (w4 + 1)),
                                            mode=self.cfg.MODEL.interpolate_mode, align_corners=True)
        ...
        if self.cfg.MODEL.LSTM:
            ...
            if self.cfg.MODEL.upsample_layer == 'deconv':
                ...
                h5, w5 = dsn5.shape[2:]
                if self.cfg.MODEL.interpolate_mode == 'nearest':
                    dsn5_up = F.interpolate(dsn5, size=(16 * (h5 + 1), 16 * (w5 + 1)),
                                            mode=self.cfg.MODEL.interpolate_mode)
                elif self.cfg.MODEL.interpolate_mode == 'bilinear':
                    dsn5_up = F.interpolate(dsn5, size=(16 * (h5 + 1), 16 * (w5 + 1)),
                                            mode=self.cfg.MODEL.interpolate_mode, align_corners=True)
        ...
        # { lstm
            ...
            # set BDCN supervision mode
            o1, o2, o3, o4, o5 = dsn1_final.detach(), dsn2_final.detach(), dsn3_final.detach(), dsn4_final.detach(), dsn5_final.detach()
            # self.cfg.MODEL.supervision = 'd2s'  # 2d2s change weight
            if self.cfg.MODEL.supervision == 's2d':
                p1_1 = dsn1_final
                p2_1 = dsn2_final + o1
                p3_1 = dsn3_final + o2 + o1
                p4_1 = dsn4_final + o3 + o2 + o1
                p5_1 = dsn5_final + o4 + o3 + o2 + o1
            elif self.cfg.MODEL.supervision == 'd2s':  # add d2s
                p1_1 = dsn1_final + o2 + o3 + o4 + o5
                p2_1 = dsn2_final + o3 + o4 + o5
                p3_1 = dsn3_final + o4 + o5
                p4_1 = dsn4_final + o5
            ...
            # { add fusion layer to see whether it can boost the performance
            # concat = torch.cat( (dsn1_final, dsn2_final, dsn3_final, dsn4_final, dsn5_final), 1 )
            # dsn6_final = self.new_score_weighting( concat )
            # end }
            # return dsn1_final, dsn2_final, dsn3_final, dsn4_final, dsn5_final, dsn6_final
        # end }
            ...
            if self.cfg.MODEL.ClsHead:
                ...
                return dsn1_final, dsn2_final, dsn3_final, dsn4_final, dsn5_final, dsn6_final, dsn7_final
